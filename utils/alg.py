...
def chuliu_edmonds(s):
  ...
  #  noncycle = torch.nonzero(noncycle.gt(0)).squeeze()#
    ...
    def contract(s):
        ...
        #s[cycle][:,noncycle]:环外点到环内点的分值。 s_cycle.view(-1, 1)：环内点的分值。[1,2]1是第一个到第二个的分值
        # s_cycle.sum():环内所有的分值

        # find the best noncycle head for each cycle dependent
        heads = s_head.argmax(0)#找出环外每个点到环内最好的入点
        ...
        s[:-1, -1] = s_dep[range(len(deps)), deps]#为新的点设置点到环外各个点的弧值，也是保留最大的那个。
        # set the contracted graph scores of cycle's potential heads
        s[-1, :-1] = s_head[heads, range(len(heads))]#为所有点设置到新的点的弧值，是改变后的最大的那个。


        '''
        deps:找出环内每个点到环外最大分数的一个点的索引。
        heads:#找出环外每个点到环内最好的入点的索引
        s:压缩环后改变了的新分数
        '''
        return s, heads, deps
...
def mst(scores, mask, multiroot=False):
    ...
    for i, length in enumerate(mask.sum(1).tolist()):
        ...
        #roots = torch.nonzero(tree[1:].eq(0))[0] + 1
